...
function page_header($page_title = '', $display_online_list = false, $item_id = 0, $item = 'forum')
{
	...
	*/
    ...
    //-- Group Leader variable
    $sql = 'SELECT g.group_id
        FROM (' . GROUPS_TABLE . ' g, ' . USER_GROUP_TABLE . ' ug)
        WHERE ug.user_id = ' . $user->data['user_id'] . '
            AND ug.group_leader = 1
            AND g.group_id = ug.group_id
            AND g.group_founder_manage = 0';
    $result = $db->sql_query_limit($sql, 1, 0);
    if ($group = $db->sql_fetchrow($result))
    {
        $template->assign_var('U_GROUP_LEADER', append_sid("{$phpbb_root_path}ucp.$phpEx", array('i' => 'groups', 'mode' => 'manage')));
    }
    //-- End Group Leader link 
